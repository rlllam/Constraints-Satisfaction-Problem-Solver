...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for vi in qi.domain():
            for vj in qj.domain():
                sameCol = (vi == vj)
                sameDia = (abs(vi - vj) == abs(i - j))
                
                if sameCol or sameDia:
                    continue
                
                sat_assignments.append([vi, vj])
                sat_assignments.append([vj, vi])

        TableConstraint.__init__(self, name, scope, sat_assignments)
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def finalTestfn(l):
            required = []
            for v in l:
                if v[1] == self._required:
                    required.append(v[1])
            return self._lb <= len(required) <= self._ub
        ...
        def partialTestfn(l):
            ...
            for v in l:
                if v[1] == self._required:
                    required.append(v[1])
            return len(required) <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        return (findvals(varsToAssign, [(var, val)], finalTestfn, partialTestfn))
